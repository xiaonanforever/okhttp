...
final class Relay {
  ...
  /** The number of bytes consumed from {@link #upstream}. */
  private long upstreamPos;

  /** True if there are no further bytes to read from {@code upstream}. */
  ...
  /** The thread currently reading from upstream, or null if no thread is doing that. */
  private Thread upstreamReader;

  /** A buffer for {@code upstreamReader} to use when pulling bytes from upstream. */
